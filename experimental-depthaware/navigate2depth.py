...
# --- MODIFIED: Import our custom local processor ---
from video_depth_processor import VideoDepthProcessor
...
def get_depth_for_box(depth_map, box):
    ...
    if x1 >= x2 or y1 >= y2: return float('inf')
...
def get_robot_instruction(detections, depth_map, frame_width, config):
    detected_fire_objects, detected_smoke_objects = [], []
    if detections and detections[0].boxes is not None:
        for r in detections:
            for box in r.boxes:
                if float(box.conf[0]) < config['yolo']['model_confidence']: continue
                cls, obj_box = int(box.cls[0]), box.xyxy[0]
                obj_center_x = (obj_box[0] + obj_box[2]) / 2
                obj_depth = get_depth_for_box(depth_map, obj_box)
                obj_data = {'center_x': obj_center_x, 'confidence': float(box.conf[0]), 'box': obj_box, 'depth': obj_depth}
                if cls == config['class_ids']['fire']: detected_fire_objects.append(obj_data)
                elif cls == config['class_ids']['smoke']: detected_smoke_objects.append(obj_data)
    ...
    if not target_objects: return config['commands']['stop'], "No Threat (Stop)", None, None
    target = max(target_objects, key=lambda x: x['confidence'])
    if target['depth'] < config['depth_model']['safe_distance_threshold']: return config['commands']['stop'], f"{threat_type} Too Close! Stop!", target['box'], target['depth']
    if target['center_x'] < config['frame_division']['left_end'] * frame_width: return config['commands']['left'], f"{threat_type} Left", target['box'], target['depth']
    elif target['center_x'] > config['frame_division']['right_start'] * frame_width: return config['commands']['right'], f"{threat_type} Right", target['box'], target['depth']
    else: return config['commands']['forward'], f"{threat_type} Center", target['box'], target['depth']
...
async def main():
    ...
    # --- MODIFIED: Initialize our custom processor ---
    depth_processor = VideoDepthProcessor(config)
    ...
    try:
        while True:
            ...
            # --- MODIFIED: Get depth map from our processor ---
            depth_map = depth_processor.process_frame(image_pil)

            # --- Decision making using the latest available depth map ---
